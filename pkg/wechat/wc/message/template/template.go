...
import (
	...
	"errors"
	"fmt"
	"github.com/zhanghup/go-framework/pkg/wechat/wc"
	"github.com/zhanghup/go-framework/pkg/wechat/wc/util"
	"net/http"
	"strconv"
	"strings"
)
...
func List(offset uint, count uint, token string) (list []Template, total uint, err error) {
	return templates(wc.BaseURL+listAPI, offset, count, token)
}
...
func Selves(offset uint, count uint, token string) (list []Template, total uint, err error) {
	return templates(wc.BaseURL+selvesAPI, offset, count, token)
}
...
func templates(api string, offset, count uint, token string) (list []Template, total uint, err error) {
	...
	if res.StatusCode != 200 {
		err = errors.New(wc.WeChatServerError)
		return
	}
...
func Get(id, token string) (keywords []KeywordItem, err error) {
	api, err := util.TokenAPI(wc.BaseURL+detailAPI, token)
	if err != nil {
		return
	}
...
func Add(id, token string, keywordIDList []uint) (string, error) {
	api, err := util.TokenAPI(wc.BaseURL+addAPI, token)
	if err != nil {
		return "", err
	}
	...
	if res.StatusCode != 200 {
		return "", errors.New(wc.WeChatServerError)
	}
...
func Delete(id, token string) error {
	api, err := util.TokenAPI(wc.BaseURL+deleteAPI, token)
	if err != nil {
		return err
	}
...
func Send(openid, template, page, formID string, data Message, emphasisKeyword, token string) error {
	api, err := util.TokenAPI(wc.BaseURL+sendAPI, token)
	if err != nil {
		return err
	}
